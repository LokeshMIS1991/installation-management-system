...
# ------------------------------------------
    ...
    # Dynamic multi-product list initialisation with Height x Width parameters
    if "order_products" not in st.session_state:
        st.session_state.order_products = [
            {"category": "Rolling Shutters", "sub_category": "Motorized Rolling Shutter", "custom_name": "", "height": "", "width": "", "unit": "mm", "quantity": 1}
        ]
    ...
    with col1:
        ...
        team_lead = st.text_input("Team Lead Name *", placeholder="e.g., Kabir")
    ...
    for idx, prod in enumerate(st.session_state.order_products):
        with st.container():
            ...
            # Explicit Height x Width Parameter Section
            c_height, c_width, c_unit, c_qty, c_del = st.columns([1.5, 1.5, 1.0, 1.2, 0.5])
            
            with c_height:
                prod["height"] = st.text_input(
                    "Height (H)", 
                    value=prod.get("height", ""), 
                    placeholder="e.g., 6000", 
                    key=f"prod_height_{idx}"
                )
                
            with c_width:
                prod["width"] = st.text_input(
                    "Width (W)", 
                    value=prod.get("width", ""), 
                    placeholder="e.g., 5330", 
                    key=f"prod_width_{idx}"
                )

            with c_unit:
                unit_opts = ["mm", "ft", "inch", "cm"]
                curr_unit_idx = unit_opts.index(prod.get("unit", "mm")) if prod.get("unit", "mm") in unit_opts else 0
                prod["unit"] = st.selectbox(
                    "Unit", 
                    unit_opts, 
                    index=curr_unit_idx, 
                    key=f"prod_unit_{idx}"
                )
    ...
    if st.button("Save Installation Order", use_container_width=True, type="primary"):
        ...
        if not team_lead.strip():
            ...
            for prod in st.session_state.order_products:
                ...
                # Build standardized dimension string (Width x Height Unit)
                h_val = prod.get("height", "").strip()
                w_val = prod.get("width", "").strip()
                unit_val = prod.get("unit", "mm")
                
                if h_val and w_val:
                    formatted_dim = f"{w_val}W x {h_val}H {unit_val}"
                elif w_val:
                    formatted_dim = f"{w_val} {unit_val}"
                else:
                    formatted_dim = "N/A"

                item_row = [inst_id, prod["category"], final_name, formatted_dim, int(prod.get("quantity", 1))]
